...
class PsinExtender:
    ...
    def __init__(self, eqdsk_data: dict):
        ...
        self.psin = (self.eqdsk["psi"] - self.eqdsk["simagx"]) / (
            self.eqdsk["sibdry"] - self.eqdsk["simagx"]
        )
    ...
    def _update_eqdsk_data(self):
        ...
        self.eqdsk["psi"] = (
            self.psin_modified * (self.eqdsk["sibdry"] - self.eqdsk["simagx"])
            + self.eqdsk["simagx"]
        )
    ...
    def plot(self, levels=100, ar_equal: bool = True) -> None:
        """Plot the original psin and the modified one side-by-side

        :param levels: Levels to use in contour plot, defaults to 100
        :param ar_equal: Whether to plot R, Z with equal aspect ratio
        """
        # TODO: Check that the modified psin has been calculated before this function executes
        fig, axis = plt.subplots(1, 2)

        # Plot the original psin
        axis[0].contour(self.r, self.z, self.psin, levels=levels)
        try:
            axis[0].plot(self.eqdsk["xlim"], self.eqdsk["ylim"], linewidth=3)
        except:
            axis[0].plot(self.eqdsk["rlim"], self.eqdsk["zlim"], linewidth=3)
        axis[0].plot(self.eqdsk["rbdry"], self.eqdsk["zbdry"], linewidth=3)
        axis[0].grid()
        if ar_equal:
            axis[0].set_aspect("equal")
        axis[0].set_title("Original eqdsk data")
        axis[0].set_xlabel("R")
        axis[0].set_ylabel("Z")

        # Plot the modified psin
        cs = axis[1].contour(
            self.r_modified, self.z_modified, self.psin_modified, levels=levels
        )
        try:
            axis[1].plot(self.eqdsk["xlim"], self.eqdsk["ylim"], linewidth=3)
        except:
            axis[1].plot(self.eqdsk["rlim"], self.eqdsk["zlim"], linewidth=3)
        axis[1].plot(self.eqdsk["rbdry"], self.eqdsk["zbdry"], linewidth=3)
        axis[1].grid()
        if ar_equal:
            axis[1].set_aspect("equal")
        axis[1].set_title("Modified eqdsk data")
        axis[1].set_xlabel("R")
        axis[1].set_ylabel("Z")

        # Add colourbar
        fig.colorbar(cs, label="$\psi$")
...
def run_psin_extender(geqdsk_path: str):
    """Run the psi extnder function, removing the upper half of domain and connecting flux surfaces up and over the magnetic axis

    :param geqdsk_path: Path to geqdsk file
    """
    ...
    new_filename = geqdsk_path + "_modified"
    with open(new_filename, "w") as f:
        geqdsk.write(pe.eqdsk, f)
    print("Saved modified equilibrium at " + new_filename)
